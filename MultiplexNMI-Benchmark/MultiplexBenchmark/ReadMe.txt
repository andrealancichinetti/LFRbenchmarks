...
This program runs the original benchmark algorithm multiple times and slice the original graph in several layers.
All the old options are available (see below) and two more options are available:

-Orig [number of original graphs]
-L [layers for each graph]

For instance: 
-Orig 3 -L 10

this will create 30 layers, 10 layers per graph.
Each layer will be recorder in file network_layer_0, network_layer_1, ..., network_layer_29.
The communities of each layer are in files community_layer_0, community_layer_1, ..., community_layer_29.
Please note that the files community_layer_0, ..., community_layer_9 are identical, and so are files community_layer_10, ..., community_layer_19 and so on.

Original graphs and partitions can be found in network_0, community_0, ...

***
The same output can also be found in two single files:
1. level_node_node_weight.edges
  First line is: *Intra
  Then you find:
  Layer Node Node Weight
2. level_node_cluster.clu
  Layer Node ClusterId
